...
###Field - CDD + HDD (Mean Thermal Load)
...
#Storage
Mean_TL <- matrix(NA, nrow = length(yrs), ncol = nrow(grid_locs))
...
for(y in 1:length(yrs)){
  ...
  #----------------------------------------------------------------------------#
  ###Compute the Annual Average HDD and CDD
  ...
  #Thermal Load
  Therm_Load <- CDD+HDD
  Therm_Load <- colMeans(Therm_Load)
  
  #Storing the data
  Mean_TL[y,] <- Therm_Load
}
...
p <- ggplot() +
  ...
  geom_tile(data = grid_locs, aes(x=Longitude, y = Latitude, 
                                  fill = colMeans(Mean_TL))) +
  scale_fill_gradient2(midpoint=median(colMeans(Mean_TL)),
                       low="blue", mid="white",high="red",
                       name = "Thermal \n Load") +
  ggtitle(paste0("Mean Thermal Load - Consistency Check"))
print(p)


#Clean-up for next objectives
Therm_Load <- nc_data <- t2m_land <- NULL
...
get_tl <- function(yr, block_size, Grids, thresh_temp){
  ...
  for(k in 1:length(block_size)) {
    ...
    #Setup storage
    tl_vals <- tl_dates <- list()
    
    for(j in 1:ncol(Therm_Load)){
      tl_temp <- get_block_maxima(t_series = Therm_Load[,j],
                                   st_date = paste0("01-01-", yr, " 00:00"),
                                   block_size = cur_block,
                                   Time_Step = "Hourly")
      tl_vals[[j]] <- tl_temp$Block
      tl_dates[[j]] <- as.character(tl_temp$Dates - hrs_behind)
    }
    
    #Store the results
    all_vals[[k]] <- unlist(tl_vals)
    all_dates[[k]] <- unlist(tl_dates)
  }
  ...
  out = list(Therm_Load_Values=all_vals,
             Therm_Load_Dates=all_dates)
}
...
ynew_results <- foreach(m = 1:nsim, .verbose = TRUE) %dopar% {
  ...
  yr <- m+1950
  get_tl(yr = yr, block_size = block_sizes,
          Grids = grid_locs, 
...
Thermal_Load_Site_Level <- list()
Thermal_Load_Site_Level[[1]] <- ann_max_values
Thermal_Load_Site_Level[[2]] <- ann_max_dates


##Consistency Check 
#Plotting the results - Consistency Check
pt_dt <- Thermal_Load_Site_Level[[1]][[1]]
...
p <- ggplot() +
  ...
  scale_fill_gradient2(midpoint=median(pt_dt/block_sizes[1]),
                       low="blue", mid="white",high="red",
                       name = "HDD") +
  ggtitle("Ann_Max Thermal Load \n Block Size 6 Hours - Code Consistency Check")
print(p)

#------------------------------------------------------------------------------#

CONUS_Thermal_Load_Site <- list(Mean = Mean_TL,
                                Site_Level = Thermal_Load_Site_Level)
...
#______________________________________________________________________________#
####Saving the results
save(CONUS_Thermal_Load_Site, 
     file = paste0("data/processed_data/Thermal_Load_Site_Level.RData"))
