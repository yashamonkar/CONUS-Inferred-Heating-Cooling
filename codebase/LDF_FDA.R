...
get_tl_maxima <- function(Population, Temp_grids, 
  ...
  #Compute the limits
  max_temp <- max(unlist(Thermal_Load_Mean))
  min_temp <- min(unlist(Thermal_Load_Mean))
  seq_temp <- seq(0,ceiling(max(min_temp,max_temp)), length.out = 200)
  
  #Set up the storage matrix
  dxy <- list()
  
  for(i in 1:length(Thermal_Load_Mean)){
    ds1 <- ecdf(Thermal_Load_Mean[[i]])
    
    dt_temp <- data.frame(X = seq_temp,
                      Y = 100*(ds1(seq_temp)),
                      Z = rep(i, length(seq_temp)))
    dxy[[i]] <- dt_temp
  }
  
  plot(dxy[[1]]$X,dxy[[1]]$Y, type='l', 
        ylab = "Capacity Utilization",
        xlab = "Temp (C)")
  for(i in 1:length(Thermal_Load_Mean)){
    lines(dxy[[i]]$X, dxy[[i]]$Y,  type='l')
  }
  
  #Unlist the dataset
  dxy <- bind_rows(lapply(dxy,data.frame))
