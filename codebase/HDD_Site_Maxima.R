...
n_region <- length(nerc_sf$FID)
nerc_label <- nerc_sf$NERC_Label
...
for(rto in 1:n_region){
  
  ###Subset Shapefile
  sub_region <- nerc_sf[rto,]
  
  #Get the Temperature Grids
  ...
  p <- ggplot() +
    ...
    ggtitle(paste0("Mean CDD - Consistency Check - ", nerc_label[rto]))
  print(p)
  ...
  NERC_HDD_Site[[rto]] <- list(Mean = Mean_HDD,
...
#______________________________________________________________________________#
####Saving the results
save(NERC_HDD_Site, file = paste0("data/processed_data/HDD_Site_Level.RData"))
